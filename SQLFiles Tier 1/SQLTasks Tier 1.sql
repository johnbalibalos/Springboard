...
the member name. */
SELECT DISTINCT CONCAT( m.firstname, ' ', m.surname, ' ', f.name ) AS bookings
FROM Members AS m
LEFT JOIN Bookings
USING ( memid )
...
SELECT name AS facility_name, CONCAT( m.firstname, ' ', m.surname ) AS guest_name,
CASE
WHEN memid =0
...
SELECT a.surname || ', ' || a.firstname as member,
            b.surname || ', ' || b.firstname as recommender
        FROM (
        SELECT *
        FROM Members
        WHERE recommendedby > 0) a
        INNER JOIN Members b
        ON (a.recommendedby = b.memid)
        ORDER BY member

/* Q12: Find the facilities with their usage by member, but not guests */
SELECT f.name, surname || ', ' || firstname as member, COUNT(surname || ', ' || firstname) AS bookings
        --SELECT DISTINCT surname || ', ' || firstname as member
        FROM
